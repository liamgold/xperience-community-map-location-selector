...
///
/// This class implements <see cref="IProductDataRetriever{ProductVariantIdentifier, DancingGoatProductData}"/> for <see cref="ProductData"/>
/// and is responsible for:
/// <list type="bullet">
///   <item><description>Building and executing content queries for product identifiers.</description></item>
///   <item><description>Mapping raw CMS content item data into <see cref="ProductData"/> objects.</description></item>
    ...
    where TProductData : ProductData
{
    ...
    /// to their corresponding <see cref="ProductData"/>.
    /// </returns>
    public async Task<IReadOnlyDictionary<TProductIdentifier, TProductData>> Get(IEnumerable<TProductIdentifier> productIdentifiers, string languageName, CancellationToken cancellationToken)
    {
        ...
        foreach (var productIdentifier in productIdentifiers)
        {
            ...
            var productDataDto = new ProductData
            {
